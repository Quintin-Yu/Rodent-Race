...
class Vehicle {
  ...
  void calculateCheckPoints() {
    for (int i = 0; i < (path.points.size()-1); i ++) {
      ...
      float x = ((PVector)path.points.get(i)).x;
      float x2 = ((PVector)path.points.get((i+1))).x;
      float y = ((PVector)path.points.get(i)).y;
      float y2 = ((PVector)path.points.get((i+1))).y;

      float distance =dist(x, x2, y, y2);
      println("SMD" + distance);
    }
  }
}
